...
class Context {
  ...
  isPage(page) {
    ...
    return this._allPages.has(page)
  }
  ...
  async getContents(page) {
    ...
    return new Vfile({
      ...
      cwd: "/",
      extname: "",
      data: { editor: "al", creator: "al", createdAt, updatedAt },
...
const makeProcessor = (context) => {
  ...
  const ppp = async (contents, editor, page, newPage) => {
    ...
    // if (out.data.newLinks && out.data.newLinks.size) {
    if (out.data.newLinks.size) {
      console.log("NEWLINKS")
      // FIXME: iterate over all newLinks
      const path = "/cnt"
      context.addPage(path)
      const out222 = await p(
        new Vfile({
          ...
          contents: `<p>Stub of <code>${path.slice(
            1
          )}</code> from <code><a href="/${page}">${page} (${out.data.newLinks.get(
            path
          )})</a></code> by <code>${editor}</code>.</p>`,
        })
      )

      delete out222.data.newLinks
      console.log("OUT222", JSON.stringify(out222, null, 2))
    }
